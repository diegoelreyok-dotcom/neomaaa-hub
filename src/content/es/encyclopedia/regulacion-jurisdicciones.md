# Jurisdicciones reguladoras — Anjouan vs el mundo

> [!INFO]
> Todo broker opera bajo alguna jurisdicción regulatoria. No todas valen lo mismo. NEOMAAA Markets opera bajo **Anjouan (AOFA)**, licencia L15968/N. Este documento explica qué significa eso, qué podemos decir al cliente, y qué jamás podemos decir.

<div className="neo-stat-grid">
<div className="neo-stat" data-value="L15968/N" data-label="Licencia NEOMAAA"></div>
<div className="neo-stat" data-value="AOFA" data-label="Regulador"></div>
<div className="neo-stat" data-value="1:1000" data-label="Leverage permitido"></div>
<div className="neo-stat" data-value="4 Tiers" data-label="Jerarquía global"></div>
</div>

## Qué es un regulador financiero

Un regulador financiero es una autoridad pública (o paraestatal) que **autoriza, supervisa y sanciona** a entidades que manejan dinero de terceros: bancos, brokers, gestoras de fondos, plataformas de pago.

Existen por tres razones:

1. **Proteger al cliente retail** de fraude, malas prácticas o quiebras.
2. **Proteger al sistema financiero** de riesgo sistémico.
3. **Combatir lavado de dinero y financiamiento al terrorismo** (AML/CFT).

Para un broker de forex/CFDs, el regulador dicta:
- Capital mínimo requerido para operar (desde USD 20k en offshore hasta EUR 750k+ en UE).
- Qué productos puede ofrecer, con qué leverage, a qué clientes.
- Cómo segregar los fondos del cliente.
- Qué información debe publicar (risk disclosures, % clientes que pierden).
- Cómo manejar quejas y disputas.
- Si existe fondo de compensación en caso de quiebra.

## Los 4 tiers de regulación

Dentro de la industria, los reguladores se agrupan informalmente en "tiers" según qué tan estrictos son, cuánto capital exigen, y qué protección al cliente garantizan.

<div className="neo-pyramid">
<div className="neo-pyramid-level">Tier 1 — FCA, CySEC, ASIC, FINMA, BaFin (capital EUR 730k+, leverage 1:30)</div>
<div className="neo-pyramid-level">Tier 2 — DFSA, FSRA, MAS, FSA Japón (leverage 1:50–1:100)</div>
<div className="neo-pyramid-level">Tier 3 — VFSC, FSC BVI, Belice, Bahamas (leverage 1:500–1:1000)</div>
<div className="neo-pyramid-level">Tier 4 — AOFA (NEOMAAA), Mauricio, Labuan, Santa Lucía</div>
</div>

### Tier 1 — Regulación estricta

| Regulador | País | Notas |
|-----------|------|-------|
| FCA | Reino Unido | Uno de los más estrictos. Fondo FSCS hasta GBP 85k. |
| CySEC | Chipre (UE) | MiFID II. Pasaporte europeo. Leverage máximo 1:30 retail. |
| ASIC | Australia | Reglas ESMA-like desde 2021. |
| FINMA | Suiza | Pocos brokers. Muy exigente. |
| BaFin | Alemania (UE) | Equivalente a CySEC bajo MiFID. |
| CONSOB | Italia (UE) | Idem. |
| AMF | Francia (UE) | Idem. |

**Características:**
- Capital mínimo alto (EUR 730k+).
- Leverage máximo retail: 1:30 en majors, 1:20 minors, 1:10 índices, 1:5 acciones, 1:2 cripto.
- Fondo de compensación (ej. FSCS UK GBP 85k, ICF Chipre EUR 20k).
- Warnings obligatorios: "El X% de los clientes retail pierden dinero".
- Prohibido bonos de depósito.
- Disputas resueltas por ombudsman público.

### Tier 2 — Regulación sólida pero menos estricta

| Regulador | País |
|-----------|------|
| DFSA | Dubái (DIFC) |
| FSRA | Abu Dhabi (ADGM) |
| MAS | Singapur |
| FSA | Japón |
| FMA | Nueva Zelanda |
| FSCA | Sudáfrica |
| CFTC/NFA | EEUU (con restricciones muy duras) |

Capital exigido intermedio, leverage moderado (1:50 a 1:100 retail), supervisión activa, pero sin la extensión de fondos de compensación del Tier 1.

### Tier 3 — Offshore con supervisión real

| Regulador | País |
|-----------|------|
| VFSC | Vanuatu |
| FSC BVI | Islas Vírgenes Británicas |
| IFSC / FSC | Belice |
| SVG FSA | San Vicente y Granadinas (ya no da licencias forex) |
| SCB | Bahamas |

Capital más bajo (USD 50k–500k), leverage alto (1:500–1:1000), bonos permitidos, menos warnings obligatorios, pero existe supervisión, auditorías periódicas, y proceso de licenciamiento.

### Tier 4 — Offshore con supervisión mínima

| Regulador | País |
|-----------|------|
| AOFA | Anjouan, Union of Comoros — **NEOMAAA Markets** |
| FSC | Mauricio |
| Labuan FSA | Malasia Labuan |
| St. Lucia FSRA | Santa Lucía |

Capital bajo, licenciamiento más accesible, supervisión limitada pero existente. Es donde entran la mayoría de brokers jóvenes, en expansión, o con modelos de negocio no compatibles con UE/UK.

## NEOMAAA Markets — Anjouan (AOFA)

**Entidad:** Neomaaa Ltd (IBC 15968)
**Licencia:** L15968/N emitida por Anjouan Offshore Finance Authority (AOFA)
**Jurisdicción:** Anjouan, Union of Comoros

### Qué implica en la práctica — Anjouan vs FCA

<div className="neo-compare">
<div className="neo-compare-col neo-compare-col--pro"><div className="neo-compare-title">Anjouan (NEOMAAA) — Pros</div><ul><li><strong>Leverage alto</strong> — hasta 1:1000 en majors</li><li><strong>Bonos de depósito permitidos</strong></li><li>Sin restricciones ESMA (no limitamos retail a 1:30)</li><li>Costos operativos menores</li><li>Mercados globales (LATAM, Asia, África, Medio Oriente)</li><li>Licencia real, auditada, renovación anual</li></ul></div>
<div className="neo-compare-col neo-compare-col--con"><div className="neo-compare-title">Anjouan vs FCA — Cons</div><ul><li>Menor confianza inicial del cliente</li><li><strong>Sin fondo de compensación europeo</strong> (no FSCS/ICF)</li><li>Disputas vía AOFA, no ombudsman europeo</li><li>Reputación default más baja en foros (Forex Peace Army)</li><li>Hay que construir reputación con ejecución y soporte</li></ul></div>
</div>

### Nuestra respuesta al cliente que pregunta por la regulación

> [!EXAMPLE]
> **Cliente:** "¿Ustedes son regulados? ¿Por quién?"
>
> **Respuesta correcta:**
> "Sí. Neomaaa Ltd está licenciada por la Anjouan Offshore Finance Authority, licencia L15968/N. Somos una jurisdicción offshore lo cual nos permite ofrecer condiciones competitivas como leverage hasta 1:1000, bonos de depósito y cuentas en múltiples monedas. Eso sí, queremos ser transparentes: no somos una licencia europea tipo FCA o CySEC, así que si buscás un fondo de compensación tipo FSCS, este no es el producto. Nuestra responsabilidad con tus fondos la cumplimos a través de **segregación de cuentas** (tu dinero está en cuentas bancarias separadas de las operativas del broker) y **Negative Balance Protection** en cuentas retail. Eso también está en el client agreement que vas a firmar."

Lo que NO podemos decir:

> [!DANGER]
> Afirmaciones prohibidas. Cualquiera de estas puede costarnos la licencia.
>
> - "Somos FCA." → **MENTIRA, GRAVE.**
> - "Somos CySEC." → **MENTIRA, GRAVE.**
> - "Estamos regulados igual que IG / Pepperstone / Interactive Brokers." → **MENTIRA.**
> - "Tu dinero está asegurado por el gobierno." → **MENTIRA.**
> - "Somos regulados en la UE." → **MENTIRA.**

## Comparación tabular

| Característica | Tier 1 (FCA/CySEC) | Tier 2 (DFSA/MAS) | Tier 3 (VFSC/Belize) | Tier 4 (Anjouan) |
|----------------|--------------------|--------------------|----------------------|------------------|
| Capital mínimo | EUR 730k+ | USD 500k–2M | USD 50k–500k | USD 20k–50k |
| Leverage retail máx | 1:30 | 1:100 | 1:500 | 1:1000 |
| Bonos permitidos | No | Parcial | Sí | Sí |
| Fondo de compensación | Sí (20k–85k) | Parcial | No | No |
| Segregación de fondos | Sí (estricta) | Sí | Sí | Sí |
| NBP obligatorio retail | Sí | Parcial | Opcional | Opcional (NEOMAAA SÍ) |
| Warning risk disclosure | Obligatorio | Obligatorio | Recomendado | Nosotros lo hacemos |
| Auditoría externa | Sí anual | Sí anual | Sí anual | Sí anual |
| Tiempo de licencia | 6–24 meses | 6–18 meses | 2–6 meses | 2–4 meses |

## Países que NO podemos aceptar

> [!WARNING]
> **Países restringidos:** no aceptamos clientes de USA, Canada, EEA (todos los países, incluida España), UK, Australia, Cuba, Iraq, Myanmar, North Korea, Sudan + sancionados. Lista completa y gold source: [`compliance/screening-sanciones`](/content/compliance/screening-sanciones) sección 12.5.

**Decisión Diego (Abril 2026):** "No podemos trabajar como broker con clientes de España/Europa sin las regulaciones que piden esos países." **EEA/UK/Australia/USA/Canadá están RESTRINGIDOS — NO operativos.** Bloqueados por Sumsub, rechazados por Compliance.

**Motivo resumido:** USA (CFTC/NFA — delito federal sin licencia US), EEA/UK/Australia (sin licencia tier-1 equivalente), sanciones internacionales (OFAC/ONU/UE) para Irán, Corea del Norte, Siria, Cuba, Myanmar, Sudán.

### Países objetivo (donde operamos activamente)

- **LATAM completo** (excluyendo USA/Canadá): Brasil, México, Colombia, Argentina, Chile, Perú, y resto de la región
- **CIS**: Rusia y vecinos con screening caso por caso (OFAC/UE/UK/ONU obligatorio)
- **MENA**: Emiratos Árabes Unidos, Saudi Arabia, Qatar, Turquía (excluyendo Iraq)
- **Sudeste Asiático (fase 2)**: Vietnam, Tailandia, Filipinas, Malasia, Indonesia, China (excluyendo North Korea y Myanmar)
- **África**: Sudáfrica, Nigeria, Kenya, Egipto (excluyendo Sudán)

**España queda como referencia / roadmap futuro** — cuando NEOMAAA obtenga licencia EEA equivalente se reactivará como target. Hoy NO es mercado operativo.

## Cómo explicarlo al cliente sin mentir ni defendernos

El cliente retail de LATAM NO entiende las 4 tiers regulatorias al mismo nivel de profundidad que un compliance officer. Pero sí entiende **lenguaje sencillo + datos concretos + opciones**. (Si el cliente es residente EEA/UK el flujo es distinto: **rechazo directo con template CR-26**, sin vender.)

Frases de cierre que funcionan:

> "Somos un broker offshore con licencia real. Optamos por esta jurisdicción porque nos permite darte condiciones de trading que en Europa ya no existen — leverage alto, bonos, cuentas en monedas múltiples. Tu dinero está protegido por segregación bancaria, y en cuentas retail tenés protección contra saldo negativo."

> "Si tu prioridad es que haya un fondo público que te cubra tipo FSCS, existen brokers en UK/CySEC. Son más caros de operar y te dan leverage 1:30. Depende de qué estés buscando."

> "Neomaaa Ltd es auditada anualmente por AOFA, tiene capital requerido depositado, y cualquier reclamo se puede escalar hasta el regulador. La documentación de licencia está pública en [sitio del regulador] y en nuestro sitio."

## Qué tiene que saber el equipo

> [!TIP]
> - NEOMAAA = Anjouan (AOFA), L15968/N. Offshore real, no FCA.
> - Pros: leverage 1:1000, bonos, mercados globales.
> - Cons: no fondo europeo, menor confianza default, hay que construir reputación.
> - **NUNCA** decir que somos FCA/CySEC/MAS — es mentira regulatoria.
> - Compensamos con NBP retail + segregación + soporte + educación.
> - EEUU prohibido absoluto. Países sancionados prohibidos absolutos.
> - **UE / EEA / UK / Australia / Canadá: RECHAZO directo.** NEOMAAA Markets no acepta clientes residentes en la EEA ni Reino Unido. Ver [`compliance/screening-sanciones`](/content/compliance/screening-sanciones) y el template de rechazo **CR-26** en `support/playbook`.
> - Ante duda del cliente: contar la verdad con contexto. La transparencia construye clientes de largo plazo.
